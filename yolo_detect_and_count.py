...
def main():
    vid_results_path = './results'
    test_vids_path = './videos'

    if not os.path.isdir(vid_results_path):
        os.makedirs(vid_results_path)

    yolo_names = ['yolov8s.pt']
    colors = [(random.randint(50, 255), random.randint(50, 255), random.randint(50, 255)) for _ in range(80)]

    counters = [YOLOv8_ObjectCounter(yolo_name, conf=0.60) for yolo_name in yolo_names]

    for counter in counters:
        counter.predict_video(
            video_path=os.path.join(test_vids_path, 'traffic2.mp4'),
    ...
    # Zipping the results directory
    zipf = zipfile.ZipFile('vid_results.zip', 'w', zipfile.ZIP_DEFLATED)
    for root, dirs, files in os.walk(vid_results_path):
        for file in files:
